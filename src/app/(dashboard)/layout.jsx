...
import { FaChevronDown, FaInfoCircle, FaMobileAlt, FaUserCircle } from "react-icons/fa"
import { MdHome } from "react-icons/md"
import { FaArrowDown, FaArrowUp, FaBitcoinSign, FaMicrochip, FaMoneyBillWave, FaUser, FaWallet } from "react-icons/fa6"
            ...
            <aside id="sidebar" className=" min-w-64 bg-gray-900 text-white shadow-2xl sticky top-0 h-screen z-50 sidebar-transition hidden md:flex flex-col">
                ...
                <div className="flex-1 overflow-y-auto">
                    <div className="p-5 border-b border-gray-800 bg-gray-800/50">
                        <div className="space-y-4">
                            <div className="flex items-center justify-between">
                                <h2 className="text-sm font-semibold text-gray-300">Общий баланс</h2>
                                
                                <FaInfoCircle className="text-teal-400 hover:text-teal-300 cursor-pointer" />
                            </div>
                            <div className="flex items-end justify-between">
                                <p id="sidebarBalance" className="text-2xl font-bold text-white">0.00</p>
                    ...
                    <nav className="p-3 space-y-1">
                        ...
                        <div onMouseEnter={() => setOrderHover(true)} onMouseLeave={() => setOrderHover(false)} className="relative group">
                            ...
                            <div className={`submenu ml-8 mt-1 space-y-1 ${orderHover ? "" : "hidden"} `}>
                        ...
                        <div onMouseEnter={() => setReqHover(true)} onMouseLeave={() => setReqHover(false)} className="relative group">
                            <div className="flex items-center gap-3 p-3 rounded-lg hover:bg-gray-800/30 text-gray-300 hover:text-teal-400 cursor-pointer transition-colors">
                                <FaMicrochip className="w-5 text-center" />
                                <span>Реквизиты</span>
                                <FaChevronDown className="text-xs ml-auto" />
                            </div>
                            <div className={`submenu ml-8 mt-1 space-y-1 ${reqHover ? "" : "hidden"} `}>
                                <Link href="/requisite" className="flex items-center px-3 py-2 text-sm text-gray-400 hover:bg-gray-800/30 rounded-lg">
                                    <FaArrowDown className="mr-2 text-teal-400" /><span>Pay in</span>
                                </Link>
                                <Link href="/requisite-out" className="flex items-center px-3 py-2 text-sm text-gray-400 hover:bg-gray-800/30 rounded-lg">
                                    <FaArrowUp className="text-red-400 mr-2" /><span>Pay out</span>
                                </Link>
                            </div>
                        </div>
                        <Link href="/devices" className="flex items-center gap-3 p-3 rounded-lg hover:bg-gray-800/30 text-gray-300 hover:text-teal-400 cursor-pointer transition-colors">
                            <FaMobileAlt className="w-5 text-center" />
                            <span>Устройства</span>
                        </Link>

                        <Link href="/withdraw" className="flex items-center gap-3 p-3 rounded-lg hover:bg-gray-800/30 text-gray-300 hover:text-teal-400 cursor-pointer transition-colors">
                            <FaWallet className="w-5 text-center" />
                            <span>Вывод средств</span>
                        </Link>
                        <Link href="/wallet" className="flex items-center gap-3 p-3 rounded-lg hover:bg-gray-800/30 text-gray-300 hover:text-teal-400 cursor-pointer transition-colors">
                            <FaMoneyBillWave className="w-5 text-center" />
                            <span>Кошелек</span>
                        </Link>
                        <Link href="profile" className="flex items-center gap-3 p-3 rounded-lg hover:bg-gray-800/30 text-gray-300 hover:text-teal-400 cursor-pointer transition-colors">
